...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // Step 4: Create the new enum type with CUSTOM_CURL method
    await queryInterface.sequelize.query(`
      CREATE TYPE "enum_Websites_httpMethod" AS ENUM (
        'GET', 'POST', 'HEAD', 'OPTIONS', 'PING', 'CURL_GET', 'CURL_POST', 'CUSTOM_CURL'
      );
    `);
    ...
    // Step 7: Add customCurlCommand column
    await queryInterface.addColumn('Websites', 'customCurlCommand', {
      type: Sequelize.TEXT,
      allowNull: true
    });
  },
  ...
  async down(queryInterface, Sequelize) {
    // Remove customCurlCommand column first
    await queryInterface.removeColumn('Websites', 'customCurlCommand');
    
    // Reverse the enum process
    await queryInterface.sequelize.query(`
      ALTER TABLE "Websites" 
      ALTER COLUMN "httpMethod" 
      DROP DEFAULT;
    ...
    await queryInterface.sequelize.query(`
      ...
      TYPE "enum_Websites_httpMethod" 
      USING CASE 
        WHEN "httpMethod" = 'CUSTOM_CURL' THEN 'GET'::"enum_Websites_httpMethod"
